...
contract DuperSuperSafeSafe {
    ...
    mapping(uint256 => bytes32) private secret_passphrases;
    uint256 timestamp;

    constructor(bytes32 _secret_passphrase, bytes32 _secret_passphrase_2)
        payable
    {
        owner = msg.sender;
    ...
    modifier passwordProtected(
        ...
        require(
            keccak256(
                abi.encodePacked(
                    secret_passphrases[0],
                    secret_passphrases[1],
                    timestamp
                )
            ) ==
                keccak256(
                    abi.encodePacked(
                        _secret_passphrase,
                        _secret_passphrase_2,
                        _timestamp
                    )
                ),
            "Wrong secret passphrase"
        );
        _;
    }
    ...
    function changeSecretPassphrase(
        ...
        public
        restricted
        passwordProtected(_secret_passphrase, _secret_passphrase_2, _timestamp)
    {
